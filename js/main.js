...
/*字符拼接区*/
	...
	/*常规定义区*/
	var styleTag = document.getElementById('styleTag');			//找到style标签
	var sourceBoard = document.getElementById('sourceBoard');	//插入显示自己的信息板
	var n = 0;

	 
/*源码版控制区*/	 
var controller= setInterval(put,1);
	 function put(){
			...
			if(n>0&&n<=929){
			    ...
			    styleTag.innerHTML =str.substring(0,n);//给style标签进行重新设置定义的风格
		...
		/*简历板写完之后，需要在sourceBoard中写入魔术代码*/	
	     ...
	     if(n>=1977){
	     	...
	     	drawBoard.innerHTML =marked(str.substring(929,1885));
	     		...
	     		if(n>str.length){window.clearInterval(controller); alert("显示完毕");}
	     }
